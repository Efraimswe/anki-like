...
export class Sm2Service {
  calculate(state: CardState, rating: Rating): Sm2Result {
    ...
    // Learning/relearning phases use step-based scheduling
    if (phase === 'new' || phase === 'learning' || phase === 'relearning') {
      return this.calculateLearning(state, rating);
    }

    // Review phase uses SM-2 algorithm
    return this.calculateReview(state, rating);
  }

  private calculateLearning(state: CardState, rating: Rating): Sm2Result {
    const steps = state.phase === 'relearning' ? RELEARNING_STEPS : NEW_STEPS;
    let { easeFactor, learningStep } = state;

    // Update ease factor for all ratings (SM-2 formula)
    ...
    if (rating === Rating.EASY) {
      // Skip all steps, graduate immediately with bonus interval
      const dueDate = new Date();
    ...
    if (rating === Rating.HARD) {
      // Stay on current step (repeat it)
  ...
  private calculateReview(state: CardState, rating: Rating): Sm2Result {
    const quality = rating as number;
    ...
    // Hard/Good/Easy: stay in review with new interval
    if (rating === Rating.HARD) {
      interval = Math.max(
        interval + 1440,
  ...
  /**
   * Preview what intervals each rating would produce for a given card state.
   * Returns human-readable strings like "1m", "10m", "1d", "4d".
   */
  previewIntervals(state: CardState): IntervalHints {
    const ratings = [Rating.AGAIN, Rating.HARD, Rating.GOOD, Rating.EASY] as const;
    const results: Record<string, string> = {};

    for (const rating of ratings) {
      const result = this.calculate(state, rating);
