...
export class StatisticsService {
  ...
  async getStatistics(userId: string, from?: string, to?: string) {
    const logs = await this.statisticsRepository.findReviewLogs(userId, from, to);
    ...
    return { retentionRate, totalReviews, accuracyPercent, totalTimeMinutes, dailyBreakdown };
  }
}
