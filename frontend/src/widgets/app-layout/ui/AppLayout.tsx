...
export default function AppLayout() {
	...
	return (
		<div
			ref={container}
			className='min-h-screen flex flex-col lg:flex-row bg-(--color-bg-page) text-(--color-text-primary) p-4 md:p-4 lg:py-0 lg:pr-6 lg:pl-0 lg:gap-6'
		>
			{/* Mobile Header */}
			...
			{/* Sidebar: Standard Left Rail (Desktop Only) */}
			<aside
				className={`sidebar-island opacity-0 -translate-x-24 fixed top-0 bottom-0 left-0 z-30 hidden lg:flex flex-col sidebar-glass sidebar-rail shadow-2xl overflow-hidden ${
					collapsed ? 'w-20' : 'w-64'
				}`}
			...
			>
				...
				>
					{!collapsed && (
						<span className='text-xl font-bold text-(--color-sidebar-text) heading truncate'>
							Anki-Like
						</span>
					)}
					<button
						onClick={toggle}
						className={`p-2 rounded-xl text-(--color-sidebar-text-muted) hover:text-(--color-sidebar-text) hover:bg-(--color-sidebar-active-bg) transition-all ${
							collapsed ? '' : ''
						}`}
				...
				<div className='p-4 space-y-3'>
					...
					<button
						onClick={toggleTheme}
						className={`flex items-center gap-4 w-full rounded-2xl text-(--color-sidebar-text-muted) hover:text-(--color-sidebar-text) hover:bg-(--color-sidebar-active-bg) transition-all ${
							collapsed ? 'justify-center p-3' : 'px-4 py-3'
						}`}
					>
						{theme === 'dark' ? (
					...
					{/* User Section */}
					<div
						className={`flex items-center rounded-2xl bg-(--color-sidebar-active-bg) p-3 ${
					...
					>
						...
						{!collapsed && (
							<div className='flex-1 min-w-0'>
								<p className='text-xs font-bold text-(--color-sidebar-text) truncate whitespace-nowrap'>
									{user?.displayName || user?.email}
								</p>
								<button
									onClick={signOut}
									className='text-[10px] uppercase font-bold text-(--color-sidebar-text-muted) hover:text-(--color-accent) transition-colors mt-0.5 whitespace-nowrap'
								>
									Sign Out
								</button>
							</div>
			...
			<nav className='lg:hidden fixed bottom-0 left-0 right-0 z-40 sidebar-glass flex items-center justify-around px-3 py-3 shadow-2xl animate-in fade-in slide-in-from-bottom-10 backdrop-blur-xl border-t border-white/10'>
				{navItems.map(item => (
					<NavLink
						key={item.to}
						to={item.to}
