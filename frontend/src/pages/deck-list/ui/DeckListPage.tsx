...
import { createDeck, deleteDeck, getDecks, updateDeck, type Deck } from '@/entities/deck';
...
const deckAccentStyles = [
	{
		badgeClass: 'deck-icon-medical',
		textClass: 'text-[#b43b00]',
		progressClass: 'bg-[#b43b00]',
		Icon: Cross,
	},
	{
		badgeClass: 'deck-icon-language',
		textClass: 'text-[#0b69c7]',
		progressClass: 'bg-[#0b69c7]',
		Icon: Languages,
	},
	{
		badgeClass: 'deck-icon-cognitive',
		textClass: 'text-[#22b95f]',
		progressClass: 'bg-[#22b95f]',
		Icon: Brain,
	},
	{
		badgeClass: 'deck-icon-arts',
		textClass: 'text-[#dc8a00]',
		progressClass: 'bg-[#ff9d00]',
		Icon: Landmark,
	},
	{
		badgeClass: 'deck-icon-math',
		textClass: 'text-[#5657dd]',
		progressClass: 'bg-[#5657dd]',
		Icon: Calculator,
	},
];
	...
	useGSAP(
		() => {
			if (showCreate) {
				...
				// Smoothly expand the form wrapper
				gsap.to('.create-form-wrapper', {
					height: 'auto',
					opacity: 1,
					duration: 0.5,
					ease: 'power3.out',
				...
				// Smoothly collapse the form wrapper
				gsap.to('.create-form-wrapper', {
					height: 0,
					opacity: 0,
					duration: 0.4,
	...
	return (
		<div ref={container} className='space-y-4 md:space-y-6'>
			<div className='decks-content-shell space-y-3 sm:space-y-4 md:space-y-6 lg:space-y-8'>
				...
				<div
					className='create-form-wrapper overflow-hidden'
					...
					<div className='create-form-gsap opacity-0 scale-95 premium-card p-4 md:p-6 mb-4'>
						...
						>
							...
							<div className='flex gap-2'>
								<button
									type='submit'
									className='button-primary flex-1 md:flex-none px-8 py-3 md:py-2'
								>
									Create
								</button>
								<button
				...
				<section className='pt-2 decks-section-gsap'>
					{decks.length === 0 ? (
						<EmptyState
						...
						<div className='decks-grid'>
							{decks.map((deck, index) => {
								...
								<div
									key={deck.id}
									className='deck-card deck-card-compact opacity-0 -translate-y-4 premium-card group'
								>
									{editingId === deck.id ? (
										<form
											onSubmit={e => {
										...
										<div className='deck-card-shell'>
											<div className='flex items-start justify-between gap-3'>
												<div className={`deck-card-icon ${accent.badgeClass}`}>
													<DeckIcon className='w-4 h-4' strokeWidth={2} />
												</div>
											...
											<Link to={`/decks/${deck.id}`} className='deck-card-link'>
												...
												<div className='deck-card-meta'>
													<div className='deck-card-stat'>
														<span className='deck-card-dot deck-card-dot-cards'></span>
														{deck.cardCount.toLocaleString()} Cards
													</div>
													<div className={`deck-card-stat ${deck.dueCount > 0 ? accent.textClass : 'text-(--color-success)'}`}>
														<span className={`deck-card-dot ${deck.dueCount > 0 ? 'bg-(--color-accent)' : 'bg-(--color-success)'}`}></span>
														{deck.dueCount > 0 ? `${deck.dueCount} Due` : 'Finished'}
													</div>
												</div>

												<div className='deck-card-progress'>
													<div className='deck-card-progress-labels'>
														<span>Mastery {mastery}%</span>
														<span>{deck.newCount ?? 0} New</span>
													</div>
													<div className='deck-card-progress-track'>
														<div
															className={`deck-card-progress-fill ${accent.progressClass}`}
															style={{ width: `${mastery}%` }}
														/>
													</div>
												</div>
