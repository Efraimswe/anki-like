...
export default function DeckDetail() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      const card = await createCard(id, { front: newFront.trim(), back: newBack.trim(), type: newType });
  ...
  const handleUpdate = async (cardId: string) => {
    ...
    try {
      const updated = await updateCard(cardId, { front: editFront.trim(), back: editBack.trim() });
          ...
          <form onSubmit={handleCreate} className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-4">
                 <button
                  type="button"
                  onClick={() => { setShowCreate(false); setNewFront(''); setNewBack(''); }}
      ...
      <div className="space-y-4">
        ...
        {cards.length === 0 ? (
          <EmptyState
            ...
            action={{ label: 'Craft Your First Card', onClick: () => setShowCreate(true) }}
          />
        ) : (
          <div className="grid grid-cols-1 gap-4">
            {cards.map((card) => (
              <div
                key={card.id}
                className="premium-card p-6 group hover:border-(--color-accent)"
              >
                {editingId === card.id ? (
