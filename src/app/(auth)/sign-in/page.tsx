...
export default function SignInPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (msg.includes('credentials') || msg.includes('invalid')) setError(tErr('invalidCredentials'));
      else if (msg.includes('network') || msg.includes('fetch')) setError(tErr('network'));
      else setError(tErr('invalidCredentials'));
    } finally {
      setLoading(false);
    }
  }
        ...
        <div className="premium-card p-6 md:p-10 shadow-2xl border-none">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between ml-1">
                <label htmlFor="password" className="text-xs font-bold uppercase tracking-widest text-(--color-text-muted)">{t('passwordLabel')}</label>
              </div>
              <input id="password" type="password" required value={password} placeholder={t('passwordPlaceholder')} onChange={(e) => setPassword(e.target.value)} className="w-full px-5 py-4 bg-(--color-bg-page) border border-(--color-border) rounded-[1.25rem] font-medium focus:ring-2 focus:ring-(--color-accent-ring) outline-none transition-all placeholder:text-(--color-text-tertiary)/50" />
            </div>
          ...
          <div className="mt-10 pt-8 border-t border-(--color-border) text-center">
            <p className="text-sm font-medium text-(--color-text-secondary)">
              {t('noAccount')}{' '}
              <Link href="/sign-up" className="text-(--color-accent) font-bold hover:underline ml-1">{t('signUpLink')}</Link>
            </p>
          </div>
        </div>
      </div>
