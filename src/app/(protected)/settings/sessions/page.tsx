      ...
      {loading ? (
        ...
        <div className="space-y-6 max-w-2xl">
          ...
          <div className="space-y-4">
                ...
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    <p className="font-bold text-(--color-text-primary) flex items-center gap-3">
                      ...
                      {session.isCurrent && (
                        <span className="text-[10px] font-bold uppercase tracking-widest bg-green-100 dark:bg-green-950/40 text-green-600 dark:text-green-400 px-2 py-0.5 rounded-md">Active Now</span>
                      )}
                    </p>
                    <p className="text-xs font-bold text-(--color-text-muted) uppercase tracking-wider mt-1">
                      {session.ipAddress} &middot; Last active {new Date(session.lastActiveAt).toLocaleDateString()}
                    </p>
                  </div>
                </div>
                {!session.isCurrent && (
            ...
            {sessions.length === 0 && (
              <div className="premium-card p-10 text-center text-(--color-text-muted) font-bold">No active sessions found.</div>
            )}
          </div>
        </div>
      )}
